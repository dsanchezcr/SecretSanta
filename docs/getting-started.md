...
- **ENVIRONMENT**: `local`
- **CLEANUP_SECRET**: (optional, **not set by default**) Add this manually to your `local.settings.json` if you want to test the cleanup endpoint locally. Use any string (e.g., `dev-secret-123`). The `/api/games/cleanup` endpoint validates requests via the `x-cleanup-secret` header (see [API Reference](./api-reference.md) for details).
